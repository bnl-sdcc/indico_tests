...
from indico_search_fake.entries import Author, ContributionEntry, EventEntry, SubContributionEntry


class FakeRemoteSearch(object):
...
class FakeSearchEngine(SearchEngine):
	...
	### BEGIN TEST ###
	import commands
	commands.getoutput('touch fake_search_process')
	### END TEST ###
	...
	### BEGIN TEST ###
	import commands
	commands.getoutput('touch /tmp/fake_search_make_query')
	### END TEST ###
    ...
    def _make_obj_query(self):
        if isinstance(self.obj, db.m.Category) and not self.obj.is_root:
            ...
            # XXX: The old plugin prefixed this with 'AND ' but according to the fake docs, AND is implied
