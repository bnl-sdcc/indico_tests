...
def create_dummy_room(db, dummy_avatar):
    """Returns a callable which lets you create dummy Bluejeans room occurrences"""
    def _create_room(name, extension, owner, data, **kwargs):
        ...
        db.session.flush()
        extension = BluejeansExtension(
            vc_room_id=vc_room.id,
            extension=extension,
            owned_by_id=owner.id
        )
        vc_room.bluejeans_extension = extension
        return vc_room
...
def test_room_cleanup(create_event, create_dummy_room, dummy_avatar, freeze_time, db):
    """Test that 'old' Bluejeans rooms are correctly detected"""
    ...
    for id_, (bluejeans_id, extension, evt_ids) in enumerate(((1234, 5678, (1, 2, 3, 4)),
                                                          (1235, 5679, (1, 2)),
                                                          (1235, 5679, (2,)),
                                                          (1236, 5670, (4,)),
                                                          (1237, 5671, ())), start=1):
    ...
    from indico_vc_bluejeans.task import find_old_bluejeans_rooms

    assert {r.id for r in find_old_bluejeans_rooms(180)} == {2, 3, 5}
