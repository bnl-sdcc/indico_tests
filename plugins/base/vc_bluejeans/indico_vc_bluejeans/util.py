...
def iter_user_identities(user):
    """Iterates over all existing user identities that can be used with Bluejeans"""
    from indico_vc_bluejeans.plugin import BluejeansPlugin
    providers = authenticators_re.split(BluejeansPlugin.settings.get('authenticators'))
    done = set()
...
def update_room_from_obj(settings, vc_room, room_obj):
    ...
    if room_obj.ownerName != vc_room.data['owner_identity']:
        ...
        vc_room.bluejeans_extension.owned_by_user = owner

    vc_room.data.update({
        'description': room_obj.description,
        'bluejeans_id': unicode(room_obj.roomID),
    ...
    vc_room.bluejeans_extension.extension = int(room_obj.extension)
