...
class BluejeansAdvancedFormMixin(object):
    ...
    ### BEGIN TEST ###
    test_field = StringField(_('Test'), [DataRequired()], description=_('write something here'))
    ### END TEST ###
...
class VCRoomAttachForm(VCRoomAttachFormBase, BluejeansAdvancedFormMixin):
    pass
...
class VCRoomForm(VCRoomFormBase, BluejeansAdvancedFormMixin):
    ...
    auto_mute = BooleanField(_('Auto mute'),
                             ...
                             description=_('The BluejeansDesktop clients will join the VC room muted by default '
                                           '(audio and video)'))
    ...
    def validate_owner_user(self, field):
        ...
        if not next(iter_user_identities(field.data), None):
            raise ValidationError(_("This user does not have a suitable account to use Bluejeans."))
