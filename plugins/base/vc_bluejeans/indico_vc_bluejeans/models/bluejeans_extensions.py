...
class BluejeansExtension(db.Model):
    __tablename__ = 'bluejeans_extensions'
    __table_args__ = {'schema': 'plugin_vc_bluejeans'}
    ...
    #: The user who owns the Bluejeans room
    ...
    def join_url(self):
        ...
        custom_url_tpl = BluejeansPlugin.settings.get('client_chooser_url')
        if custom_url_tpl:
    ...
    @return_ascii
    def __repr__(self):
        return '<BluejeansExtension({}, {}, {})>'.format(self.vc_room, self.extension, self.owned_by_user)


@listens_for(BluejeansExtension.owned_by_user, 'set')
