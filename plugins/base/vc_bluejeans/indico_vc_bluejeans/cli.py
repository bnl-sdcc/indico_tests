...
@cli_group(name='bluejeans')
def cli():
    """Manage the Bluejeans plugin."""


@cli.command()
@click.option('--status', type=click.Choice(['deleted', 'created']))
def rooms(status=None):
    """Lists all Bluejeans rooms"""

    room_query = VCRoom.find(type='bluejeans')
    table_data = [['ID', 'Name', 'Status', 'Bluejeans ID', 'Extension']]
    ...
    for room in room_query:
        table_data.append([unicode(room.id), room.name, room.status.name,
                           unicode(room.data['bluejeans_id']), unicode(room.bluejeans_extension.extension)])
