...
from indico_vc_bluejeans.api.cache import ZeepCache
        ...
        while True:
            ...
            for room in response.room:
                if int(room.extension) == int(extension):
                    BluejeansPlugin.logger.debug('Room: %s has been found.', room)
                    return room
                else:
                    BluejeansPlugin.logger.debug('Dismissing room extension %s', room.extension)
                counter += 1

    @raises_api_error
    def get_room(self, bluejeans_id):
        return self.soap.getRoom(roomID=bluejeans_id)
