...
class DeleteVCRoomAPI(HTTPAPIHook):
    ...
    def _has_access(self, user):
        ...
        return user in BluejeansPlugin.settings.acls.get('managers')
    ...
    def api_deletevcroom(self, user):
        from indico_vc_bluejeans.plugin import BluejeansPlugin
        from indico_vc_bluejeans.api import APIException
        ...
        for rid in self._room_ids:
            room = VCRoom.query.filter(VCRoom.type == 'bluejeans',
                                       VCRoom.status == VCRoomStatus.created,
                                       VCRoom.data.contains({'bluejeans_id': str(rid)})).first()
            if not room:
            ...
            try:
                ...
                failed.append(rid)
                BluejeansPlugin.logger.exception('Could not delete VC room %s', room)
            else:
                room.status = VCRoomStatus.deleted
                success.append(rid)
                BluejeansPlugin.logger.info('%s deleted', room)
