...
(function () {
  ...
  var ImporterUtils = {
    ...
    shortMonthsNames: [$t.gettext("Jan"), $t.gettext("Feb"), $t.gettext("Mar"), $t.gettext("Apr"), $t.gettext("May"), $t.gettext("Jun"), $t.gettext("Jul"), $t.gettext("Aug"), $t.gettext("Sep"), $t.gettext("Oct"), $t.gettext("Nov"), $t.gettext("Dec")],
    ...
    minutesToTime: function minutesToTime(minutes) {
      if (minutes <= 1440) {
        return ((minutes - minutes % 60) / 60 < 10 ? "0" + (minutes - minutes % 60) / 60 : (minutes - minutes % 60) / 60) + ":" + (minutes % 60 < 10 ? "0" + minutes % 60 : minutes % 60);
      } else {
        return '00:00';
      }
    },
     ...
     */
    compareStartTime: function compareStartTime(a, b) {
     ...
     */
    sortedKeys: function sortedKeys(dict, sortFunc) {
      var array = [];
     ...
     */
    isPersonEmpty: function isPersonEmpty(person) {
     ...
     */
    _sendRequest: function _sendRequest(url, args, onSuccess, onError) {
      $.ajax({
  ...
  type("QueueDict", [], {
     ...
     */
    shiftTop: function shiftTop(idx) {
      if (idx > 0) {
     ...
     */
    shiftBottom: function shiftBottom(idx) {
  ...
  type("ImportDialog", ["ExclusivePopupWithButtons", "PreLoadHandler"], {
     ...
     */
    _hideLists: function _hideLists() {
     ...
     */
    _showLists: function _showLists() {
    ...
    drawContent: function drawContent() {
      ...
      var _observeInsertButton = function _observeInsertButton() {
      ...
      return Html.div({}, Html.div({
        className: 'importDialogHeader',
        style: {
          width: pixels(this.width * 0.9)
        }
      }, query, searchButton, ' ', $t.gettext("in"), ' ', importFrom), this.emptySearchDiv.draw(), this.importerList.draw(), this.timetableList.draw());
    },
    _getButtons: function _getButtons() {
      var self = this;
      return [[$t.gettext('Proceed...'), function () {
    ...
    _onClose: function _onClose(evt) {
      if (this.reloadPage) {
        location.reload();
  ...
  type("PresearchContainer", [], {
     ...
     */
    showAfterSearch: function showAfterSearch() {
    ...
    draw: function draw() {
      ...
      this.contentDiv = Html.div({
        className: 'presearchContainer',
        style: {
          "height": pixels(this.height - 130)
        }
      }, this.firstSearch, this.afterSearch);
  ...
  type("ImporterDurationDialog", ["ExclusivePopupWithButtons", "PreLoadHandler"], {
     ...
     */
    function (hook) {
      var self = this; //If the destination is a contribution, simply fetch the contribution's start time.
    ...
    drawContent: function drawContent() {
      ...
      return IndicoUtil.createFormFromMap([[$t.gettext("Duration time of every inserted contribution:"), durationField], [$t.gettext("Start time of the first contribution:"), timeField], [$t.gettext("Show me the destination:"), redirectCheckbox]]);
    },

    /* Redirect to the timetable containing the created entry. */
    _performRedirect: function _performRedirect() {
    ...
    _getUrl: function _getUrl(eventId, day, destination) {
      var params = {
        'confId': eventId
      };
    ...
    _personLinkData: function _personLinkData(entry) {
      var authorType = {
        'none': 0,
        'primary': 1,
        'secondary': 2
      ...
      var linkDataEntry = function linkDataEntry(author, authorType, speaker) {
    ...
    _addContributionMaterial: function _addContributionMaterial(title, link_url, eventId, contributionId, subContributionId) {
      var requestUrl;
    ...
    _addReference: function _addReference(type, value, eventId, contributionId, subContributionId) {
      var url;
    ...
    _getButtons: function _getButtons() {
      var self = this;
      return [[$t.gettext('Insert'), function () {
        ...
        var duration = parseInt(self.info.get('duration')); //If last contribution finishes before 24:00
            ...
            var successCallback = function successCallback(result) {
              ...
              if (self.destination.entryType == 'Contribution') {
                $.each(materials, function (title, url) {
                  self._addContributionMaterial(title, url, result.event_id, result.contribution_id, result.id);
                });
                $.each(reportNumbers, function (idx, number) {
                  self._addReference(reportNumbersLabel, number, self.confId, result.contribution_id, result.id);
                });
              } else {
  ...
  type("ImporterListWidget", ["SelectableListWidget"], {
     ...
     */
    getLastImporter: function getLastImporter() {
     ...
     */
    isMoreToImport: function isMoreToImport() {
     ...
     */
    _searchBase: function _searchBase(query, importer, size, successFunc, callbacks) {
      var self = this;
      $.ajax({
     ...
     * @param callbacks List of methods executed after request (doesn't matter if successful).
     */
    search: function search(query, importer, size, callbacks) {
      var self = this;
     ...
     * @param callbacks List of methods executed after request (doesn't matter if successful).
     */
    append: function append(size, callbacks) {
      var self = this;
     ...
     */
    _getPersonString: function _getPersonString(person) {
      return person.firstName + " " + person.familyName + (person.affiliation ? " (" + person.affiliation + ")" : "");
    },

    /**
     * Draws sequence number attached to the item div
     */
    _drawSelectionIndex: function _drawSelectionIndex() {
      var self = this;
      var selectionIndex = Html.div({
      ...
      var stopMouseEvent = function stopMouseEvent(event) {
     ...
     */
    _convertMaterials: function _convertMaterials(materials) {
      var materialsDict = {};
     ...
     */
    _getResourceName: function _getResourceName(resource) {
     ...
     */
    _drawItem: function _drawItem(record) {
      var self = this;
      var recordDiv = Html.div({});
      var key = record.key;
      record = record.get(); // Empty fields are not displayed.
      ...
      if (record.get("summary")) {
        var summary = record.get("summary"); //If summary is too long it need to be truncated.

        if (summary.length < 200) {
          recordDiv.append(Html.div({}, Html.em({}, $t.gettext("Summary")), ": ", summary));
        } else {
          var summaryBeg = Html.span({}, summary.substr(0, 200));
          var summaryEnd = Html.span({
            style: {
              display: 'none'
            }
          }, summary.substr(200));
          var showLink = Html.span({
            className: 'fake-link'
          }, $t.gettext(" (show all)"));
          showLink.observeClick(function (evt) {
            summaryEnd.dom.style.display = "inline";
            showLink.dom.style.display = "none";
            hideLink.dom.style.display = "inline"; //Preventing event propagation

            evt.cancelBubble = true; //Recalculating position of the selection number
          ...
          hideLink.observeClick(function (evt) {
            ...
            hideLink.dom.style.display = "none"; //Preventing event propagation
     ...
     */
    observeSelection: function observeSelection(list) {
      var self = this; //Clears numbers next to the all divs
  ...
  function (events, listStyle, selectedStyle, customObserver) {
    var self = this; // After selecting/deselecting an element two observers are executed.
  ...
  type("ImporterList", [], {
     ...
     */
    _appendCallbacks: function _appendCallbacks(callbacks) {
      var self = this;
    ...
    _drawHeader: function _drawHeader() {
      this.entriesCount = Html.span({}, '0');
      this.descriptionDiv = Html.div({
        className: 'entryListDesctiption'
      }, this.entriesCount, $t.gettext("Please select the results you want to insert."));
      this.emptyDescriptionDiv = Html.div({
        className: 'entryListDesctiption'
      }, $t.gettext("No results were found. Please change the search phrase."));
      return Html.div({}, Html.div({
        className: 'entryListHeader'
      }, $t.gettext("Step 1: Search results:")), this.descriptionDiv, this.emptyDescriptionDiv);
    },
    _drawImporterDiv: function _drawImporterDiv() {
      var self = this;
      ...
      }, this.importerWidget.draw(), this.moreEntriesDiv);
    }
  },
  /**
  ...
  type("TimetableListWidget", ["ListWidget"], {
     ...
     */
    setSelection: function setSelection(selected, div) {
      if (this.selectedDiv) {
     ...
     */
    _drawItem: function _drawItem(item, level) {
      var self = this;
      level = level ? level : 0; // entry is a Day
      ...
      switch (item.entryType) {
        ...
        case undefined:
          ...
          }, item.key.substr(6, 2) + " " + ImporterUtils.shortMonthsNames[parseFloat(item.key.substr(4, 2)) - 1] + " " + item.key.substr(0, 4));
      ...
      var itemDiv = Html.div({
        style: {
          marginLeft: pixels(level * 20),
          clear: "both",
          padding: pixels(5)
        }
      }, titleDiv);
      var entriesDiv = Html.div({
        style: {
          display: "none"
        }
      }); //Draws subentries
     ...
     */
    _drawShowHideButtons: function _drawShowHideButtons(div) {
      var showButton = Html.img({
        src: imageSrc("collapsd"),
        style: {
          display: 'block'
        }
      });
      var hideButton = Html.img({
     ...
     */
    _insertFromTimetable: function _insertFromTimetable() {
      var self = this;
...
/******/ });
//# sourceMappingURL=main.03b93444.bundle.js.map