...
(function (global) {
  ...
  type('IntelligentSearchBox', ['RealtimeTextBox'], {
    _highlight: function _highlight(elem, token) {
    ...
    _truncateTitle: function _truncateTitle(title) {
      var max = 27;
    ...
    _showSuggestions: function _showSuggestions(text, suggList, totalNumber) {
      var self = this;
      ...
      if (!this.suggestionBox) {
        ...
        var infoBox = Html.div('help', $t.gettext("Some category suggestions...")); // Create the suggestion box
        ...
        }, infoBox, this.suggestionList, $B(Html.div('other-results'), this.totalNumber, function (number) {
      ...
      var tokenRE = new RegExp('(' + tokens.join('|') + ')', 'gi'); // Bind the list to the DOM element
      ...
      $B(this.suggestionList, suggList, function (elem) {
        ...
        var liItem = Html.li({}, title, path); // Use mouse to control selector as well
    ...
    _retrieveOptions: function _retrieveOptions(expression) {
      var self = this;
      this.querying = true;
      $.ajax({
        url: self.searchUrl,
        ...
        success: function success(data) {
          ...
          var currentText = trim(self.get()); // if the text changed meanwhile and it
    ...
    _getTimeSinceLastQuery: function _getTimeSinceLastQuery() {
      var now = new Date();
      return now.getTime() - this.timeOfLastQuery;
    },
    _waitForRequestTime: function _waitForRequestTime() {
      var self = this;
     ...
     */
    _textTyped: function _textTyped(key) {
      var self = this;
    ...
    _openSelection: function _openSelection(event) {
     ...
     */
    _moveSelector: function _moveSelector(direction) {
      if (this.suggestionBox) {
    ...
    _setSelector: function _setSelector() {
      if (this.selectorPos >= 0) {
        this.suggestionList.item(this.selectorPos).dom.className = 'selected';
      }
    },
    _clearSelector: function _clearSelector() {
      if (this.selectorPos >= 0) {
  ...
  $.widget('indico.search_tag', {
    ...
    _transition: function _transition(title, no_check) {
      var $tag = this.$tag;
      var old_width = $tag.width();
      $tag.fadeTo('fast', 0.3).width('').find('.where').html(title);
      var new_width = $tag.width(); // store target width
    ...
    _create: function _create() {
      ...
      var tag_template = _.template('<div class="search-tag">' + '<div class="cross">x</div>' + '<div class="where"><%= categ_title %></div>' + '</div>');
    ...
    confirm_tip: function confirm_tip() {
      ...
      this.options.form.attr('action', this.options.search_category_url); // use target-width as search-tag may still be growing
    ...
    search_everywhere: function search_everywhere() {
      $('.cross', this.$tag).hide();
    ...
    show_categ: function show_categ() {
      var $tag = this.$tag;
      var $where = $('.where', $tag);
      var self = this;
    ...
    show_tip: function show_tip(event) {
      this.$tag.qtip({
    ...
    back_to_everywhere: function back_to_everywhere() {
      var $where = $('.where', this.$tag);
      var self = this;
  ...
  global.categorySearchBox = function categorySearchBox(options) {
    ...
    function verifyForm() {
      ...
      return (!startDate || Util.parseDateTime(startDate, IndicoDateTimeFormats.DefaultHourless)) && (!endDate || Util.parseDateTime(endDate, IndicoDateTimeFormats.DefaultHourless));
    }
...
/******/ });
//# sourceMappingURL=main.1f687a96.bundle.js.map