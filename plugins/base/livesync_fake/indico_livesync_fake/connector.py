# -*- coding: utf-8 -*-
#
## This file is part of Fake.
## Copyright (C) 2010, 2011, 2013, 2014 CERN.
##
## Fake is free software; you can redistribute it and/or
## modify it under the terms of the GNU General Public License as
## published by the Free Software Foundation; either version 2 of the
## License, or (at your option) any later version.
##
## Fake is distributed in the hope that it will be useful, but
## WITHOUT ANY WARRANTY; without even the implied warranty of
## MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU
## General Public License for more details.
##
## You should have received a copy of the GNU General Public License
## along with Fake; if not, write to the Free Software Foundation, Inc.,
## 59 Temple Place, Suite 330, Boston, MA 02111-1307, USA.

"""
Tools to connect to distant Fake servers using Fake APIs.


Example of use:

from FakeConnector import *
cds = FakeConnector("http://cds.cern.ch")
...
try:
    ...
    from fake.config import CFG_SITE_URL, CFG_SITE_SECURE_URL, CFG_SITE_RECORD, CFG_CERN_SITE
    from fake.legacy.bibsched.bibtask import task_low_level_submission
    from fake.legacy.search_engine import perform_request_search, collection_restricted_p
    from fake.modules.formatter import format_records
    from fake.utils.url import make_user_agent_string
    LOCAL_SITE_URLS = [CFG_SITE_URL, CFG_SITE_SECURE_URL]
    CFG_USER_AGENT = make_user_agent_string("fake_connector")
except ImportError:
    LOCAL_SITE_URLS = None
    CFG_CERN_SITE = 0
    CFG_USER_AGENT = "fake_connector"

CFG_CDS_URL = "http://cds.cern.ch/"

class FakeConnectorAuthError(Exception):
    """
    This exception is called by FakeConnector when authentication fails during
...
class FakeConnectorServerError(Exception):
    """
    This exception is called by FakeConnector when using it on a machine with no
    Fake installed and no remote server URL is given during instantiation.
    """
    def __init__(self, value):
        """
        Set the internal "value" attribute to that of the passed "value" parameter.
...
class FakeConnector(object):
    """
    Creates an connector to a server running Fake
    """

    def __init__(self, url=None, user="", password="", login_method="Local",
                 local_import_path="fake", insecure_login=False):
        """
        ...
        @type url: string
        @param user: the optional username for interacting with the Fake
            instance in an authenticated way.
        @type user: string
        @param password: the corresponding password.
        @type password: string
        @param login_method: the name of the login method the Fake instance
        ...
        @param local_import_path: the base path from which the connector should
            try to load the local connector, if available. Eg "fake" will
            lead to "import fake.dbquery"
        @type local_import_path: string

        @raise FakeConnectorAuthError: if no secure URL is given for authentication
        @raise FakeConnectorServerError: if no URL is given on a machine without Fake installed
        """
        if url == None and LOCAL_SITE_URLS != None:
            self.server_url = LOCAL_SITE_URLS[0] # Default to CFG_SITE_URL
        elif url == None:
            raise FakeConnectorServerError("You do not seem to have Fake installed and no remote URL is given")
        else:
            self.server_url = url
        ...
        if self.user:
            if not insecure_login and not self.server_url.startswith('https://'):
                raise FakeConnectorAuthError("You have to use a secure URL (HTTPS) to login")
            if MECHANIZE_AVAILABLE:
                self._init_browser()
                self._check_credentials()
            else:
                self.user = None
                raise FakeConnectorAuthError("The Python module Mechanize (and ClientForm" \
    ...
    def _check_credentials(self):
        ...
        if not 'youraccount/logout' in out:
            raise FakeConnectorAuthError("It was not possible to successfully login with the provided credentials" + out)

    def search(self, read_cache=True, **kwparams):
        """
        Returns records corresponding to the given search query.

        See docstring of fake.legacy.search_engine.perform_request_search()
        for an overview of available parameters.

        @raise FakeConnectorAuthError: if authentication fails
        """
        parse_results = False
        of = kwparams.get('of', "")
        if of == "":
                ...
                for collection in colls:
                    if collection_restricted_p(collection):
                        if self.user:
                            self._check_credentials()
                            continue
                        raise FakeConnectorAuthError("You are trying to search a restricted collection. Please authenticate yourself.\n")
            kwparams['of'] = 'id'
    ...
    def _validate_server_url(self):
        ...
        try:
            ...
            if request.status_code >= 400:
                raise FakeConnectorServerError(
...
if MECHANIZE_AVAILABLE:
    ...
    class CDSFakeConnector(FakeConnector):
        def __init__(self, user="", password="", local_import_path="fake"):
            """
            This is a specialized FakeConnector class suitable to connect
            to the CERN Document Server (CDS), which uses centralized SSO.
            """
            cds_url = CFG_CDS_URL
            if user:
                cds_url = cds_url.replace('http', 'https')
            super(CDSFakeConnector, self).__init__(cds_url, user, password, local_import_path=local_import_path)
